...
def validate(net, testloader, criterion, device, is_segmentation=False):
    ...
    shape_ious = 0.0
    count = 0.0
    # CatmIoU = torch.zeros(16)
    # Catcount = torch.zeros(16)
    with torch.no_grad():
        for batch_idx, data_dic in enumerate(tqdm(testloader)):
            ...
            if is_segmentation:
                ...
                batch_ious = data_dic['pred_score_logits'].new_tensor([np.sum(miou)],
                                                                      dtype=torch.float64)  # same device with seg_pred

                # CatmIoU[data_dic['cls_tokens'][0][0]] += miou[0]
                # print(miou[0])
                # Catcount[data_dic['cls_tokens'][0][0]] += 1
                ...
                accuracy.append(correct.item() / (data_dic['pred_score_logits'].shape[0] * data_dic['points'].shape[
                    1]))  # append the accuracy of each iteration
                mious.append(miou)
        ...
        # print(CatmIoU)
        if is_segmentation:
            # Cat_miou = torch.mean(CatmIoU / Catcount)
            overall_miou = np.mean(mious)
            overall_acc = np.mean(accuracy)
            overall_ins_acc = shape_ious * 1.0 / count
            return {
                "cat.miou": float("%.3f" % (100. * Cat_miou)),
                "ins.miou": float("%.3f" % (100. * overall_miou)),
                "time": time_cost,
