...
class PointStackSeg(nn.Module):
    def __init__(self, cfg, activation="relu", bias=False, use_xyz=False, normalize="anchor", **kwargs):
        ...
        self.embedding = ConvBNReLU1D(6, cfg.NETWORK.ENCODER.EMBED_DIM, bias=bias, activation=activation)
        ...
        # self.skip_connect = PCT(in_channel=en_dims[-1], out_channel=en_dims[-1])
        ...
        for i in range(len(en_dims)-1):
            ...
            self.skip_connect.append(
                PCT(in_channel=en_dims[i+1], out_channel=en_dims[i+1])
            )
    ...
    def forward(self, data_dic):
        ...
        for i in range(len(self.decode_list)):
            ...
            x_list[i + 1] = self.skip_connect[i](x_list[i+1])
...
class PCT(nn.Module):
    ...
    def forward(self, x):
        ...
        x_global_feature = torch.concat((x_max_feature, x_avg_feature), 1)  # 1024 + 64
