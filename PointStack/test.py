...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default='cfgs/partnormal/pointstack.yaml', help='specify the config for training')
    parser.add_argument('--ckpt', type=str, default='experiments/PartNormal/experiments/ckpt/ckpt-best.pth', help='checkpoint to start from')
...
if cfg.DATASET.IS_SEGMENTATION:
    Catmiou = np.round(val_dict['cat.miou'], 4)
    insmiou = np.round(val_dict['ins.miou'], 4)
    print("cat.miou is", Catmiou)
    print("ins.miou is", insmiou)
...
if cfg.DATASET.IS_SEGMENTATION:
    with open(exp_dir + '/eval_best.txt', 'w') as f:
        ...
        f.write('\nBest cat.miou: ' + str(Catmiou))
        f.write('\nBest ins.miou: ' + str(insmiou))
