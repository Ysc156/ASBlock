...
import sys
sys.path.append("/home/yansh/anaconda3/envs/pt/lib/python3.7/site-packages/pointops-0.0.0-py3.7-linux-x86_64.egg/")
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    elif args.arch == 'pointtransformer_seg_repro_skip':
        from model.pointtransformer.pointtransformer_seg_skip import pointtransformer_seg_repro as Model
    elif args.arch == 'skip':
        from model.pointtransformer.skip import pointtransformer_seg_repro as Model
    else:
    ...
    if args.resume0:
        if os.path.isfile(args.resume0):
            if main_process():
                logger.info("=> loading checkpoint '{}'".format(args.resume0))
            checkpoint = torch.load(args.resume0, map_location=lambda storage, loc: storage.cuda())
            model.load_state_dict(checkpoint['state_dict'], strict=False)
    if args.resume:
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        #output, skip_out = model([coord, feat, offset])
        output = model([coord, feat, offset])
        if target.shape[-1] == 1:
            target = target[:, 0]  # for cls
        #lambda_ = 0.1**(0.1*epoch)
        lambda_ = max(0, (3600-epoch**2)/3600)
        
        #loss = criterion(output, target) + lambda_ * criterion(skip_out, target)
...
def validate(val_loader, model, criterion):
    ...
    for i, (coord, feat, target, offset) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            output = model([coord, feat, offset])
        loss = criterion(output, target)
        #loss = criterion(skip_out, target)
