...
from models.curvenet_seg_skip import CurveNet
import numpy as np
...
def train(args, io):
    ...
    try:
        checkpoint = torch.load('../checkpoints/%s/models/model.t7' % args.exp_name)
        checkpoint_last = torch.load('../checkpoints/%s/models/model_last.t7' % args.exp_name)
        opt.load_state_dict(checkpoint_last['opt'])
        scheduler.load_state_dict(checkpoint_last['scheduler'])
        start_epoch = checkpoint_last['epoch'] + 1
        model.load_state_dict(checkpoint_last['model_state_dict'])
        io.cprint('Use pretrain model')
        best_test_iou = checkpoint['inctance_avg_iou']
    ...
    for epoch in tqdm(range(start_epoch, args.epochs + 1)):
        ...
        io.cprint('best_test_iou == %.4f  best_epoch == %d' % (best_test_iou, best_epoch))
        ####################
        # Train
        ####################
        train_loss = 0.0
        ...
        io.cprint('Saving the last model...')
        torch.save(state, '../checkpoints/%s/models/model_last.t7' % args.exp_name)
        if test_ious >= best_test_iou:
            best_test_iou = test_ious
            best_epoch = epoch
            io.cprint(('best iou %.4f' % best_test_iou))
            state_best = {
                'epoch': epoch,
                'train_acc': train_acc,
                'inctance_avg_iou': best_test_iou,
                'model_state_dict': model.state_dict(),
                'best_epoch': epoch
            }
            torch.save(state_best, '../checkpoints/%s/models/model.t7' % args.exp_name)
            io.cprint('Saving the best model...')
...
def test(args, io):
    ...
    for key in category.keys():
        ...
        CatmIoU.append(np.mean(category[key]))
    results.sort(key=lambda x: x[0])
    CatmIoU = np.mean(CatmIoU)
    io.cprint('cat.mIoU: %.3f' % (CatmIoU))
    for re in results:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str, default='curveunet_seg_1', metavar='N',
    ...
    parser.add_argument('--model_path', type=str, default='checkpoints/curveunet_seg_1/models/model.t7', metavar='N',
