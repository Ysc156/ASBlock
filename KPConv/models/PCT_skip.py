...
class PCT(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv2 = nn.Conv1d(5, trans_dim, kernel_size=1, bias=False)
        ...
        self.conv_fuse = nn.Sequential(nn.Conv1d(trans_dim*4, 32, kernel_size=1, bias=False),
                                       nn.GroupNorm(4, 32),
                                       nn.LeakyReLU(negative_slope=0.2))

        self.label_conv = nn.Sequential(nn.Conv1d(trans_dim, 64, kernel_size=1, bias=False),
                                        nn.GroupNorm(4, 64),
                                        nn.LeakyReLU(negative_slope=0.2))

        self.convs1 = nn.Conv1d(32 * 3, 512, 1)
    ...
    def forward(self, x, x_global):
        ...
        x1 = self.sa1(x, x_global)
        # print('x1', x1)
        x2 = self.sa2(x1, x_global)
        x3 = self.sa2(x2, x_global)
        x4 = self.sa2(x3, x_global)
...
class SA_Layer(nn.Module):
    ...
    def forward(self, x_q, x_kv):
        x_q = self.q_conv(x_q).permute(0, 2, 1)  # b, n1, c
        # print('xkv', x_kv)
        x_k = self.k_conv(x_kv)  # b, c, n2
        x_v = self.v_conv(x_kv)  # b, c, n2
        # print('xk', x_k)
